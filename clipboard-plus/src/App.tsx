...
import type { ClipboardEntry, GroupKey } from './types';
import { t } from './i18n';
...
export default function App() {
  ...
  const filtered = useMemo(
    () => search
      ? grouped.filter(e => e.content.toLowerCase().includes(search.toLowerCase()))
      : grouped,
    [grouped, search]
  );
  ...
  const handleClearAll = useCallback(async () => {
    await clearHistory();
    setSelectedId(null);
    showToast(t('notify.cleared'));
  }, [clearHistory, showToast]);

  const handleSaveSettings = useCallback(async (s: Partial<typeof settings>) => {
    if (settings) {
  ...
  return (
    <div className="app">
      ...
      <div className="toolbar">
        <button className="toolbar-btn" onClick={() => setSettingsOpen(true)} title={t('context.settings')}>
          ☰
        </button>
        <SearchBar value={search} onChange={setSearch} />
        <button className="toolbar-btn danger" onClick={handleClearAll} title={t('context.clear_all')}>
          ✕
        </button>
      </div>
      ...
      <div className="status-bar">
        ...
        <span>
          <kbd>{hotkeyDisplay}</kbd> {t('footer.hotkey', { hotkey: '' }).replace('{hotkey}', '')}
        </span>
      </div>
